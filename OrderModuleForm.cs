...
namespace InventoryManagementSystem
{
    public partial class OrderModuleForm : Form
    {
        ...
        public void LoadProduct()
        {
            ...
            cm = new SqlCommand("SELECT pid, pname, wp, pdescription, pcategory FROM tbProduct WHERE CONCAT(pid, pname, pprice, pdescription, pcategory) LIKE '%" + txtSearchProd.Text + "%'", con);
            con.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            e.Graphics.DrawImage(Properties.Resources.modern_sports_shoes_logo_vector_36922249, 5, 5, 200, 200);
            float yPos = 8;
            int leftMargin = 15;
            ...
            //======================================                      
            string nam, price, qut, tot,  Num;

            yPos += 40;
            e.Graphics.DrawString("همسه ", PatientFontheader, Brushes.Purple, leftMargin + 630, yPos, new StringFormat());
            e.Graphics.DrawString(" فاتورة مببعات  ", PatientFontheader1, Brushes.Purple, leftMargin + 330, yPos, new StringFormat());
            yPos += 60;
            e.Graphics.DrawString("لجميع انواع الاحذية الحريمي ", PatientFontheader2, Brushes.Purple, leftMargin + 500, yPos, new StringFormat());
            yPos += 60;
            e.Graphics.DrawString(dtOrder.Text + ": التاريخ ", PatientFontheader1, Brushes.Purple, leftMargin + 580, yPos, new StringFormat());
            yPos += 40;
            e.Graphics.DrawString("---------------------------------------------------------" + " : مطلوب من السيدة او الانسة ", PatientFontheader1, Brushes.Purple, leftMargin + 0, yPos, new StringFormat());
            yPos -= 10;
            e.Graphics.DrawString(textBox1.Text, PatientFontheader1, Brushes.Purple, leftMargin + 200, yPos, new StringFormat());
            yPos += 45;
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            try
            {
                ...
                if (textBox1.Text == "")
                {
                    MessageBox.Show("من فضلك ادخل اسم العميل", "خطأ", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                if (MessageBox.Show("هل انت متاكد من اضافه العنصر؟", "جاري الحفظ", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    cm = new SqlCommand("INSERT INTO tbOrder(odate, pid, cid, qty, price, total,name)VALUES(@odate, @pid, @cid, @qty, @price, @total,@name)", con);
        ...
        private void button2_Click_1(object sender, EventArgs e)
        {
            int i = 0;
            dgvProduct.Rows.Clear();
            cm = new SqlCommand("SELECT * FROM tbProduct WHERE pname=@username ", con);
            cm.Parameters.AddWithValue("@username", txtSearchProd.Text);
            con.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
