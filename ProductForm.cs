...
namespace InventoryManagementSystem
{
    public partial class ProductForm : Form
    {
        ...
        public void LoadProduct()
        {
            ...
            cm = new SqlCommand("SELECT * FROM tbProduct WHERE CONCAT(pid, pname, pprice,Q1,wp,pdescription, pcategory) LIKE '%" + txtSearch.Text + "%'", con);
            con.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
        ...
        private void dgvProduct_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (colName == "Edit")
            {
                ...
                productModule.tp.Text = dgvProduct.Rows[e.RowIndex].Cells[5].Value.ToString();
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            int i = 0;
            dgvProduct.Rows.Clear();
            cm = new SqlCommand("SELECT * FROM tbProduct WHERE pdescription=@username ", con);
            cm.Parameters.AddWithValue("@username", txtSearch.Text);
            con.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
