...
namespace InventoryManagementSystem
{
    public partial class OrderForm : Form
    {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            int i = 0;
            dgvOrder.Rows.Clear();
            cm = new SqlCommand("SELECT iddd, odate, O.pid, P.pname, O.cid, name, qty, price, total  FROM tbOrder AS O JOIN tbCustomer AS C ON O.cid=C.cid JOIN tbProduct AS P ON O.pid=P.pid ", con);
            con.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
